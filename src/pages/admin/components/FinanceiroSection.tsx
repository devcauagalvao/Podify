    ...
    <section className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h2 className="flex items-center gap-2 text-sm font-bold text-ink-900">
          <DollarSign size={16} className="text-brand-500" /> Financeiro
        </h2>
        <button
          onClick={recarregar}
          disabled={loading}
          className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 transition hover:text-ink-900 disabled:opacity-50"
        >
      ...
      {loading && !dados ? (
        ...
        <div className="rounded-2xl border border-rose-100 bg-rose-50 p-5 text-sm text-rose-700">{erro}</div>
      ) : dados ? (
        <>
          <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
            ...
            <div className="card p-4 sm:p-5">
              ...
              <p className="mt-1 text-2xl font-extrabold text-emerald-600">{formatMoeda(dados.recebidoMes)}</p>
            </div>
            <div className="card p-4 sm:p-5">
              <p className="text-xs text-slate-500">Assinantes</p>
              <p className="mt-1">
                <span className="text-xl font-extrabold text-ink-900">{dados.assinantesPagantes}</span>{' '}
                <span className="text-sm text-slate-500">pagantes</span>
              </p>
              <p className="text-xs text-slate-400">{dados.assinantesCortesia} com acesso cortesia</p>
            </div>
          </div>
